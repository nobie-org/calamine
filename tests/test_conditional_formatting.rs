use calamine::{
    open_workbook, CfvoType, Color, ComparisonOperator, ConditionalFormatRule,
    ConditionalFormatType, IconSetType, TimePeriod, Xlsx,
};
...
fn test_conditional_formatting_parsing() {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/test_conditional_formatting.xlsx");

    let mut workbook: Xlsx<_> = open_workbook(&path).expect("Cannot open file");

    // Get conditional formatting for the worksheet
    let cf_rules = workbook
        .worksheet_conditional_formatting("Conditional Formatting")
    ...
    assert_eq!(
        cf_rules.len(),
        6,
        "Expected 6 conditional formatting blocks"
    );
    ...
    for cf_block in cf_rules.iter() {
        for rule in &cf_block.rules {
            assert!(
                !rules_by_priority.contains_key(&rule.priority),
    ...
    for priority in 1..=20 {
        assert!(
            rules_by_priority.contains_key(&priority),
            "Missing rule with priority {}",
            priority
        );
    }
        ...
        ConditionalFormatType::ColorScale(color_scale) => {
            ...
            match &color_scale.colors[1] {
                Color::Theme {
                    ...
                    assert!(
                        (tint + 0.249977111117893).abs() < 0.0001,
                        "Expected tint value -0.249977111117893, got {}",
                        tint
                    );
                }
    ...
    println!(
        "- DataBar showValue attribute on outer element is not parsed when dataBar is a child"
    );
...
fn test_parse_without_conditional_formatting() {
    ...
    assert!(
        cf_rules.is_empty(),
        "Expected no conditional formatting rules"
    );
}
...
fn test_conditional_format_type_display() {
    ...
    assert_eq!(
        TimePeriod::AllDatesInJanuary.to_string(),
        "allDatesInPeriodJanuary"
    );
    assert_eq!(
        TimePeriod::AllDatesInQ1.to_string(),
