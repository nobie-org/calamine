...
pub struct PivotTable {
    ...
    /// The location/extent of the pivot table
    pub location: Dimensions,
    ...
    /// References to fields used as row fields
    pub row_fields: Vec<PivotFieldReference>,
    /// References to fields used as column fields
...
pub struct PivotItem {
    /// The resolved value of this item (string representation)
    pub value: String,
...
pub struct PivotField {
    ...
    /// Field items with detailed information
...
/// Reference to a field in pivot table layouts
#[derive(Debug, Clone, PartialEq)]
pub enum PivotFieldReference {
    /// Regular field index (0-based)
    FieldIndex(u32),
    /// Placeholder field (index -1)
    Placeholder,
    /// Values/Data area (index -2)
    Values,
}

impl PivotFieldReference {
    /// Create a field reference from an integer index
    pub fn from_index(index: i32) -> Option<Self> {
        match index {
            -2 => Some(Self::Values),
            -1 => Some(Self::Placeholder),
            n if n >= 0 => Some(Self::FieldIndex(n as u32)),
            _ => None,
        }
    }

    /// Convert field reference back to integer index
    pub fn to_index(&self) -> i32 {
        match self {
            Self::FieldIndex(idx) => *idx as i32,
            Self::Placeholder => -1,
            Self::Values => -2,
        }
    }

    /// Check if this is a regular field index
    pub fn is_field_index(&self) -> bool {
        matches!(self, Self::FieldIndex(_))
    }

    /// Get the field index if this is a regular field reference
    pub fn field_index(&self) -> Option<u32> {
        match self {
            Self::FieldIndex(idx) => Some(*idx),
            _ => None,
        }
    }
}
