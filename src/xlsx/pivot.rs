...
use crate::pivot::{
    ...
    PivotFieldDataType, PivotFieldReference, PivotFieldType, PivotItem, PivotSourceType,
    PivotTable, PivotTableInfo,
};
use crate::{Data, Dimensions, Reader, XlsxError};
        ...
        loop {
            ...
            match reader.read_event_into(&mut buf) {
                Ok(Event::Start(ref e)) => match e.local_name().as_ref() {
                        ...
                        loop {
                            ...
                            match reader.read_event_into(&mut inner_buf) {
                                ...
                                Ok(Event::End(ref inner_e))
                                    if inner_e.local_name().as_ref() == b"pivotField" =>
                                {
                                    break;
                                }
                                Ok(Event::Eof) => break,
                    ...
                    b"rowFields" => {
                        pivot.row_fields = parse_field_references(&mut reader, b"rowFields")?;
                    }
                    b"colFields" => {
                        pivot.column_fields = parse_field_references(&mut reader, b"colFields")?;
                    }
                    b"dataFields" => {
                        pivot.data_fields = parse_data_fields(&mut reader)?;
                    }
                    _ => {}
                },
        ...
        if pivot.source_range.is_none() {
            ...
            for cache in self.pivot_tables.caches.values() {
                if cache.source_range.is_some() {
                            ...
                            for item in &mut field.items {
                                if item.value.is_empty() {
                                    item.value = if let Some(index) = item.cache_index {
                                        ...
                                        if let Some(shared_item) =
                                            cache_field.shared_items.get(index as usize)
                                        {
                                            match shared_item {
                    ...
                    // Update data field names from cache
                    for data_field in &mut pivot.data_fields {
                        if let Some(cache_field) = cache.fields.get(data_field.field_index as usize)
                        {
                            data_field.name = cache_field.name.clone();
                        }
                    }
                ...
                b"cacheField" => {
                    ...
                    loop {
                        ...
                        match reader.read_event_into(&mut inner_buf) {
                            ...
                            Ok(Event::End(ref inner_e))
                                if inner_e.local_name().as_ref() == b"cacheField" =>
                            {
                                break;
                            }
                            Ok(Event::Eof) => {
                                return Err(XlsxError::Unexpected("Unexpected EOF in cacheField"))
                            }
                            Err(e) => return Err(XlsxError::Xml(e)),
                            _ => {}
                        }
                ...
                if e.attributes().count() > 0 {
                    match e.local_name().as_ref() {
                        ...
                        b"b" => {
                            ...
                            if let Some(v_attr) = get_attribute(e.attributes(), QName(b"v"))? {
                                let v_str = reader.decoder().decode(v_attr)?;
                                items.push(Data::Bool(
                                    v_str == "1" || v_str.to_lowercase() == "true",
                                ));
                            }
                        }
                        b"m" => {
...
/// Parse field references (for rowFields/colFields)
fn parse_field_references<RS: Read + Seek>(
    reader: &mut XlReader<'_, RS>,
    section_name: &[u8],
) -> Result<Vec<PivotFieldReference>, XlsxError> {
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) | Ok(Event::Empty(ref e)) => {
                if e.local_name().as_ref() == b"field" {
                    if let Some(x_attr) = get_attribute(e.attributes(), QName(b"x"))? {
                        let x_str = reader.decoder().decode(x_attr)?;
                        if let Ok(field_index) = x_str.parse::<i32>() {
                            if let Some(field_ref) = PivotFieldReference::from_index(field_index) {
            ...
            Ok(Event::End(ref e)) if e.local_name().as_ref() == section_name => {
                break;
            }
            Ok(Event::Eof) => break,
            Err(e) => return Err(XlsxError::Xml(e)),
...
/// Parse data fields
fn parse_data_fields<RS: Read + Seek>(
    reader: &mut XlReader<'_, RS>,
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) | Ok(Event::Empty(ref e)) => {
                if e.local_name().as_ref() == b"dataField" {
                    let mut data_field = PivotDataField {
                        name: String::new(),
                        field_index: 0,
                    ...
                    // Get field index
                    if let Some(fld_attr) = get_attribute(e.attributes(), QName(b"fld"))? {
                        let fld_str = reader.decoder().decode(fld_attr)?;
                        if let Ok(index) = fld_str.parse::<u32>() {
                            data_field.field_index = index;
                        }
                    }

                    // Get subtotal/aggregation function
                    if let Some(subtotal_attr) = get_attribute(e.attributes(), QName(b"subtotal"))?
                    {
                        let subtotal_str = reader.decoder().decode(subtotal_attr)?;
                        data_field.aggregation = AggregationFunction::from_str(&subtotal_str)
                    ...
                    if let Some(name_attr) = get_attribute(e.attributes(), QName(b"name"))? {
                        data_field.display_name =
                            Some(reader.decoder().decode(name_attr)?.into_owned());
                    }

                    data_fields.push(data_field);
                }
            }
            Ok(Event::End(ref e)) if e.local_name().as_ref() == b"dataFields" => {
                break;
            }
            Ok(Event::Eof) => break,
            Err(e) => return Err(XlsxError::Xml(e)),
